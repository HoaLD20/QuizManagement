    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (q.getContent().equals("") || !hasCharacter(q.getContent())) {
                ...
                error = 7;
            } else if (q.getOption().get(0).equals(q.getOption().get(1)) || q.getOption().get(0).equals(q.getOption().get(2))
                    || q.getOption().get(0).equals(q.getOption().get(3))
                    || q.getOption().get(1).equals(q.getOption().get(2)) || q.getOption().get(1).equals(q.getOption().get(3))
                    || q.getOption().get(2).equals(q.getOption().get(3))) {
                error = 8;
            }
            if (error == 0) {
            ...
            if (error == 2) {
                req.setAttribute("message", "Content of option can not empty!");
            }
            if (error == 3) {
                req.setAttribute("message", "Content of option can not empty!");
            }
            ...
            if (error == 6) {
                req.setAttribute("message", "Please select answer for this option!");
            }
            if (error == 7) {
                req.setAttribute("message", "Cannot select all answers!");
            }
            if (error == 8) {
                req.setAttribute("message", "Answer can not be same !");
            }
